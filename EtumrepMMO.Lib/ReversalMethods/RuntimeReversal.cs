...
public static class RuntimeReversal
{
    ...
    public static (ulong Seed, byte Rolls)[] GetSeeds(PKM pk, byte max_rolls)
    {
        if (pk.IsShiny)
        {
            Program.main.etumrepdisplay.AppendText("Shiny...\n");
        ...
        if (UseNativeReversalLibrary && RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            ...
            Program.main.etumrepdisplay.AppendText("Falling back to runtime search...\n");
        }
    ...
    {
        Program.main.etumrepdisplay.AppendText("Searching with runtime search method...\n");
        var sw = new Stopwatch();
        sw.Start();
        var result = GetAllSeeds(pk, max_rolls);
        var msg = result.Count == 0 ? "No seeds found." : "Found seeds using runtime search method:";
        Program.main.etumrepdisplay.AppendText(msg+"\n");
        ...
        for (int i = 0; i < result.Count; i++)
        {
            ...
            Program.main.etumrepdisplay.AppendText($"{seed:x} (rolls = {shinyRolls})\n");
        }

        Program.main.etumrepdisplay.AppendText($"Time: {sw.Elapsed.TotalSeconds:F2} seconds\n");
    ...
    private static void FindPotentialSeeds(ConcurrentBag<ulong> all_seeds, uint pid, uint ec, byte max_rolls)
    {
        ...
        Program.main.etumrepdisplay.AppendText($"Iteration start seed: {start_seed:X16}\n");
